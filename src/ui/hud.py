...
class HUD:
    ...
    def draw(
        ...
        try:
            ...
            cv2.putText(
                frame, f"Reps / Time: {reps}", (10, y), self.font, 0.8, (0, 255, 255), 2
            )
            y += line_gap

            if level is not None:
                cv2.putText(
                    frame, f"Level: {level}", (10, y), self.font, 0.7, (0, 255, 255), 2
                )
                y += line_gap
            ...
            cv2.putText(
                frame, f"State: {state}", (10, y), self.font, 0.8, (255, 255, 0), 2
            )
            y += line_gap
    ...
    def show_badges(self, badges):
        now = time.time()
        for badge in badges:
            self.active_badges.append({"badge": badge, "start_time": now})
    ...
    def draw_badge_popups(self, frame):
        ...
        for entry in self.active_badges:
            if now - entry["start_time"] < 3.0:
                ...
                cv2.rectangle(frame, (20, y - 30), (500, y + 10), (0, 140, 255), -1)
